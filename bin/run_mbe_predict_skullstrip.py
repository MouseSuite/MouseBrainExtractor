...
def parser():
    parser = argparse.ArgumentParser(description='Skull stripping using Mouse Brain Extractor.')
    parser.add_argument('--j', help="Input JSON file name for batch processing.", required=False)
    parser.add_argument('-i', help="Input MRI file name.", required=False)
    ...
    parser.add_argument('-n', help='Model file that contains pre-trained weights.', required=True)
    parser.add_argument('-p', help="If global absolute positional encodings have already been generated, you can specify it using this flag.", required=False)
    ...
    parser.add_argument('--pp', help="Run post-processing on the masks.", required=False, action='store_true')
    parser.add_argument('--copy_input', help="Copy input to output directory for easy access.", required=False, action='store_true')
    parser.add_argument('--dstype', help="In vivo or ex vivo. Arugment for RAP BIDS App.", choices=['invivo', 'exvivo'], required=False)
    ...
    parser.add_argument('--gen_posenc', help="Generate positional encoding files prior to running prediction.", required=False)
...
def main():
    ...
    if args.dstype == 'invivo' and args.d == 3:
        dstype = 'invivo_iso'
    elif args.dstype == 'invivo' and args.d == 2:
        dstype = 'invivo_aniso'
    elif args.dstype == 'exvivo' and args.d == 3:
        dstype = 'exvivo'
    ...
    if args.i:
        ...
        if use_pos:
            if args.p:
                posfile = args.p
            elif args.gen_posenc:
                posfile = args.gen_posenc
                if args.m == 'mod5':
